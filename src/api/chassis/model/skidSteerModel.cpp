...
namespace okapi
{
  SkidSteerModel::SkidSteerModel(std::shared_ptr<AbstractMotor> ileftSideMotor,
                                 ...
                                 const double imaxVoltage, std::shared_ptr<ContinuousRotarySensor> igyro)
      : maxVelocity(imaxVelocity),
        ...
        rightSensor(std::move(irightEnc)), gyro(igyro)
  {
  }
  ...
  {
    return std::valarray<std::int32_t>{0,0,0};
   // auto thing = gyro->get() * 100.0;
   // thing = 0;
   // printf("HIII %f\n", thing);
   // return std::valarray<std::int32_t>{static_cast<std::int32_t>(leftSensor->get()),
   //                                    static_cast<std::int32_t>(rightSensor->get()), static_cast<std::int32_t>(thing)};
  }
